...
from eval.PerceptualSimilarity.models import dist_model as dm
...
class FID:
    ...
    def calculate_from_disk(self, generated_path, gt_path):
        ...
        print("calculate real_path statistics...")
        m1, s1 = self.compute_statistics_of_path(gt_path, self.verbose)
    ...
    def compute_statistics_of_path(self, path, verbose):
        ...
        if os.path.exists(npz_file):
            ...
            imgs = np.array([imread(str(fn)).astype(np.float32) for fn in tqdm(files)])
...
class Reconstruction_Metrics:
    ...
    def __call__(self, inputs, gts):
        ...
        if hasattr(self, "psnr"):
            psnr_value = peak_signal_noise_ratio(inputs, gts, self.data_range)
    ...
    def calculate_from_disk(self, inputs, gts, save_path=None, sort=True, debug=0):
        ...
        if os.path.exists(npz_file):
            ...
            for index in tqdm(range(len(input_image_list))):
                name = os.path.basename(input_image_list[index])
                names.append(name)

                img_gt_256 = imread(str(gt_image_list[index]))
                img_pred_256 = imread(str(input_image_list[index]))
                img_gt = img_gt_256.astype(np.float32) / 255.0
                img_pred = img_pred_256.astype(np.float32) / 255.0
                ...
                psnr.append(
                    peak_signal_noise_ratio(
                        img_gt, img_pred, data_range=self.data_range
                    )
                )
                ssim.append(
...
class LPIPS:
    ...
    def calculate_from_disk(
        self, path_1, path_2, batch_size=64, verbose=False, sort=True
    ):
        if sort:
        ...
        imgs_1 = np.array(
            [imread(str(fn)).astype(np.float32) / 127.5 - 1 for fn in files_1]
        )
        imgs_2 = np.array(
            [imread(str(fn)).astype(np.float32) / 127.5 - 1 for fn in files_2]
        )
        ...
        for i in range(n_batches):
            ...
            result.append(
                self.model.forward(img_1_batch, img_2_batch).detach().cpu().numpy()
            )
    ...
    def calculate_mask_lpips(
        ...
        for i in range(len(files_1)):
            ...
            mask = np.tile(
                np.expand_dims(create_masked_image(string).astype(np.float32), -1),
                (1, 1, 3),
            )  # .repeat(1,1,3)
            imgs_1.append(
                (imread(str(files_1[i])).astype(np.float32) / 127.5 - 1) * mask
            )
            imgs_2.append(
                (imread(str(files_2[i])).astype(np.float32) / 127.5 - 1) * mask
            )
...
def produce_ma_mask(kp_array, img_size=(128, 64), point_radius=4):
    ...
    for f, t in limbs:
        from_missing = (
            kp_array[f][0] == MISSING_VALUE or kp_array[f][1] == MISSING_VALUE
        )
...
def load_pose_cords_from_strings(y_str, x_str):
    ...
    return np.concatenate(
        [np.expand_dims(y_cords, -1), np.expand_dims(x_cords, -1)], axis=1
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--fid_real_path", help="Path to real images when calculate FID", type=str
    )
    ...
    gt_list, distorated_list = preprocess_path_for_deform_task(
        args.gt_path, args.distorated_path
    )
    print("calculate reconstruction metric...")
    rec_dic = rec.calculate_from_disk(
        distorated_list,
        gt_list,
        save_path=args.distorated_path,
        sort=False,
        debug=False,
    )
    ...
    if args.calculate_mask:
        mask_lpips_score = lpips.calculate_mask_lpips(
            distorated_list, gt_list, sort=False
        )
